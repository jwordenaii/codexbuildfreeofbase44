    ...
    faqSchema([
      {
        ...
        answer:
          `Yes. This future-ready page model supports ${model.name} paving content for services, pricing, seasonality, compliance, and Google-quality checks before it is added to the sitemap.`,
      },
      {
    ...
    <>
      ...
      <section className="bg-brand-navy text-white py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <h1 className="font-display font-black text-4xl md:text-6xl mt-3 mb-5">
            {model.h1}
          </h1>
          <p className="text-white/70 text-xl max-w-3xl">
            This page framework is ready for tomorrow&rsquo;s state expansion. It is
            intentionally built with helpful-content controls so every state page can
            become useful, original, and Google-policy aligned before sitemap publishing.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 mt-8">
            <Link to="/quote" className="btn-primary">Request a Free Estimate</Link>
            <Link to="/services" className="btn-outline">View Asphalt Services</Link>
          </div>
        </div>
      </section>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {model.targetServices.map((service) => (
                  <div key={service} className="rounded-xl border border-brand-navy/10 p-4 text-sm text-brand-navy/75">
                    ✓ {service}
                  </div>
                ))}
              </div>
              ...
              <ul className="space-y-3">
                {model.contentRequirements.map((requirement) => (
                  <li key={requirement} className="flex gap-3 text-sm text-brand-navy/75">
                    <span className="w-5 h-5 rounded-full bg-brand-amber text-brand-navy font-bold text-xs flex items-center justify-center flex-shrink-0">✓</span>
                    {requirement}
                  </li>
                ))}
              </ul>
          ...
          <aside className="space-y-5">
            <div className="rounded-2xl bg-brand-navy text-white p-6">
              ...
              <p className="text-white/60 text-sm">
                Policy reviewed {readiness.lastPolicyReviewed}. Sitemap publishing is
                controlled separately so future states can be prepared without thin-page risk.
              </p>
            </div>
            ...
            <div className="rounded-2xl border border-brand-navy/10 p-6">
              ...
              <ul className="space-y-2 text-sm text-brand-navy/70">
                <li>Price index: {model.laborIndex} labor / {model.materialPremium} materials</li>
                <li>Paving season: {model.asphaltMonths} months</li>
                <li>QSR density: {model.qsrDensity}</li>
                {model.complianceSignals.map((signal) => <li key={signal}>✓ {signal}</li>)}
              </ul>
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap gap-2">
            {nearbyStates.map((state) => (
              <Link key={state.abbr} to={state.path} className="bg-white border border-brand-navy/10 rounded-full px-4 py-2 text-sm text-brand-navy/70 hover:border-brand-amber">
                {state.name}
              </Link>
            ))}
          </div>
